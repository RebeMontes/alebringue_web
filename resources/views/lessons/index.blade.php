...
<!--         
        <div class="stat-card p-4">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-ale-text-dim text-sm">Publicadas</p>
                    <p class="text-2xl font-bold text-ale-text">{{ $lessons->where('status', 'published')->count() }}</p>
                </div>
                ...
                <tbody class="divide-y divide-ale-border">
                    ...
                    <tr class="lesson-row hover:bg-ale-surface/50 transition-colors" data-title="{{ strtolower($lesson->title) }}" data-level="{{ $lesson->level->name ?? '' }}" data-status="{{ $lesson->status ?? 'published' }}">
                        <td class="px-4 py-4 align-middle">
                            <input type="checkbox" class="lesson-checkbox rounded border-ale-border bg-transparent" value="{{ $lesson->id }}">
                        </td>
                        <td class="px-4 py-4 align-middle">
...
<script>
    ...
    function filterTable() {
        ...
        rows.forEach(row => {
            ...
            const status = row.dataset.status || 'published';
    ...
    document.getElementById('btn-exportar')?.addEventListener('click', function() {
        ...
        visibleRows.forEach(row => {
            const title = row.querySelector('td:nth-child(3) .font-semibold')?.textContent || '';
            const level = row.querySelector('td:nth-child(4) span')?.textContent.trim() || '';
            const statusElem = row.querySelector('td:nth-child(5) span');
            let status = '';
            if (statusElem?.textContent.includes('Publicada')) status = 'published';
            else if (statusElem?.textContent.includes('Borrador')) status = 'draft';
            
            data.push({ title, level, status });
        });
        
        const csv = data.map(row => `"${row.title}","${row.level}","${row.status}"`).join('\n');
        const blob = new Blob([`Título,Nivel,Estado\n${csv}`], { type: 'text/csv;charset=utf-8;' });
    ...
    selectAll?.addEventListener('change', function() {
        lessonCheckboxes.forEach(cb => cb.checked = selectAll.checked);
    });
</script>
@endpush