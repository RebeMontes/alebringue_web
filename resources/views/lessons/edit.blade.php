...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-ale-surface border border-ale-border rounded-xl p-6">
        <h2 class="text-xl font-bold text-white mb-6 flex items-center gap-2">
            ...
            Editar Lección: {{ $lesson->title }}
        </h2>

        <form method="POST" action="{{ route('lessons.update', $lesson->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          class="w-full p-3 rounded-lg bg-black/50 border border-ale-border text-white focus:border-ale-pink focus:outline-none transition resize-none">{{ old('description', $lesson->description) }}</textarea>
                @error('description')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <!-- <div>
                ...
                <select name="status" 
                        class="w-full p-3 rounded-lg bg-black/50 border border-ale-border text-white focus:border-ale-pink focus:outline-none transition">
                    <option value="draft" {{ old('status', $lesson->status) == 'draft' ? 'selected' : '' }}>Borrador</option>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                    ...
                    <label class="flex items-center gap-3 p-2 rounded-lg bg-black/30 border border-ale-border hover:border-ale-pink transition cursor-pointer">
                        <input type="checkbox" 
                               name="words[]" 
                               value="{{ $word->id }}"
                               {{ in_array($word->id, old('words', $lesson->words->pluck('id')->toArray())) ? 'checked' : '' }}
    ...
    <div class="mt-4 p-4 bg-ale-surface/50 border border-ale-border rounded-lg">
        <div class="flex items-center gap-3 text-sm">
            ...
            <p class="text-gray-400">Creada: {{ $lesson->created_at ? $lesson->created_at->format('d/m/Y H:i') : 'N/A' }} | ID: {{ $lesson->id }}</p>
        </div>
    </div>
