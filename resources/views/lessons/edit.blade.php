        ...
        <form method="POST" action="{{ route('lessons.update', $lesson->id) }}" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          class="w-full p-3 rounded-lg bg-black/50 border border-ale-border text-white focus:border-[#00E5FF] focus:outline-none transition resize-none">{{ old('description', $lesson->description) }}</textarea>
                @error('description')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
