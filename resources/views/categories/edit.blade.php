        ...
        <form method="POST" action="{{ route('categories.update', $category->id) }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          class="w-full p-3 rounded-lg bg-black/50 border border-ale-border text-white focus:border-ale-pink focus:outline-none transition resize-none">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="bg-ale-surface/50 rounded-lg p-3 text-sm">
                <div class="flex items-center gap-2 text-gray-400">
                    ...
                    @if($category->words)
                        <span class="mx-2">|</span>
                        <span><i class="fas fa-language"></i> {{ $category->words->count() }} palabras</span>
                    @endif
                </div>
            </div>
